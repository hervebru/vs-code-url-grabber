...
function copyCurrentFilePathWithCurrentLineNumber(markdown: boolean = false, includeHighlightedTextAsCodeBlock: boolean = false): string {
	...
	const config = vscode.workspace.getConfiguration('hbruCodeUrlGrabber');
	...
	if (remoteName === 'ssh-remote') {
		...
		if (sshRemoteFqdn) {
			...
			if (serverIp) {
				vscodeUri = `vscode-remote/${remoteName}%2B${serverIp}`;
			} else {
				vscodeUri = 'file';
			}
		}
		...
		if (distroName) {
			vscodeUri = `vscode-remote/wsl%2B${distroName}`;
		} else {
			vscodeUri = 'file';
		}
	} else if (remoteName) { // Fallback to unknown remote type
		vscodeUri = `vscode-remote/${remoteName}`;
	} else if (scheme === 'file') {
		vscodeUri = 'file';
	} else {
		vscodeUri = `${scheme}/${authority}`;
	}

	console.log(`[Code URL Grabber] document.uri.scheme: ${scheme}`);
	console.log(`[Code URL Grabber] document.uri.authority: ${authority}`);
	console.log(`[Code URL Grabber] document.uri.toString(): ${document.uri.toString()}`);
	console.log(`[Code URL Grabber] vscode.env.remoteName: ${remoteName}`);
	console.log(`[Code URL Grabber] SSH_CONNECTION: ${process.env.SSH_CONNECTION}`);
	console.log(`[Code URL Grabber] WSL_DISTRO_NAME: ${process.env.WSL_DISTRO_NAME}`);
	...
	const url = `${protocol}://${vscodeUri}${path}:${lineNumber}${includeColumn ? `:${columnNumber}` : ''}`;
	console.log(`[Code URL Grabber] Generated URL: ${url}`);
...
export function activate(context: vscode.ExtensionContext) {
	...
	outputChannel = vscode.window.createOutputChannel('Code URL Grabber', { log: true });
	context.subscriptions.push(outputChannel);
	const version = context.extension.packageJSON.version;
	const extensionKind = context.extension.extensionKind;

	let extensionKindInfo: string;
	if (extensionKind === vscode.ExtensionKind.UI) {
		extensionKindInfo = 'ExtensionKind = UI (Extension runs where the UI runs)';
	} else if (extensionKind === vscode.ExtensionKind.Workspace) {
		extensionKindInfo = 'ExtensionKind = Workspace (Extension runs where the remote extension host runs)';
	} else {
		extensionKindInfo = `ExtensionKind = ${extensionKind}`;
	}

	console.log(`Extension "hbru-vs-code-url-grabber" v${version} is now active.`);
	console.log(`[Code URL Grabber] ${extensionKindInfo}`);
	outputChannel.info(`Extension "hbru-vs-code-url-grabber" v${version} is now active.`);
	outputChannel.info(`${extensionKindInfo}`);
	//outputChannel.show(true);

	let copyRawLink = vscode.commands.registerCommand('hbru-vs-code-url-grabber.copyLink', () => {
	...
	let copyMarkdownLink = vscode.commands.registerCommand('hbru-vs-code-url-grabber.copyMarkdownLink', () => {
		let filePathWithLineNumber;
		try {
	...
	let copyLinkAndSelection = vscode.commands.registerCommand('hbru-vs-code-url-grabber.copyLinkAndSelection', () => {
	...
	let copyMarkdownLinkAndSelection = vscode.commands.registerCommand('hbru-vs-code-url-grabber.copyMarkdownLinkAndSelection', () => {
		let filePathWithLineNumberAndCode;
		try {
